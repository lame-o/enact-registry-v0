            ...
            <div className="flex-1">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant="outline" className="border-gray-600 text-gray-300">ENACT v{task.version}</Badge>
                    {task.isAtomic && (
                      <Badge variant="secondary" className="bg-gray-700 text-gray-200">atomic</Badge>
                    )}
                    {task.teams.map((team) => (
                      <Badge key={team} variant="secondary" className="bg-gray-700 text-gray-200">{team}</Badge>
                    ))}
                  </div>
                </div>
                <CollapsibleTrigger className="p-1 hover:bg-[#1f2937] rounded-md transition-colors">
                  <Icon icon="lucide:chevron-down" className="w-5 h-5 text-gray-400 transition-transform group-open:rotate-180" />
                </CollapsibleTrigger>
              </div>
              <CardDescription className="mt-1 text-gray-300">{task.description}</CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent>
                    ...
                    <div className="flex items-start gap-2">
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge variant="outline" className="border-gray-600 text-gray-300">{input.type}</Badge>
                        </div>
                    ...
                    <div className="flex items-start gap-2">
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          {t.language && (
                            <Badge variant="outline" className="border-gray-600 text-gray-300">{t.language}</Badge>
                          )}
                        </div>
                        {t.description && (
                  ...
                  <div key={key} className="bg-[#1f2937] rounded-md p-2">
                    <div className="flex items-start gap-2">
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          <span className="text-white font-medium">{key}</span>
                          <Badge variant="outline" className="border-gray-600 text-gray-300">{output.type}</Badge>
                        </div>
                        {output.description && (
